...
variable_id = 9302
variable_to_compare_id = 8017


def get_pipeline_url(id):
    return f"{organization_url}/{project_name}/_apis/distributedtask/variablegroups/{id}?api-version={api_version}"
...
url = get_pipeline_url(variable_id)
response = get_request_data(url)
response_to_compare = get_request_data(get_pipeline_url(variable_to_compare_id))
